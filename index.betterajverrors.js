...
function doValidate(schemajson, json, validated_version, file, str, success, failure) {
	let retval = false;
	...
	if (typeof validated_version !== 'undefined') {
		...
		if (!valid) {
			...
			const output = betterAjvErrors(schemajson, json, errs, { "format" : "js", "json": str });
  			// console.log(output);
			failure(output);
		} else {
...
function loadSchema(json, file, str, doValidate, success, failure) {
	...
	let validated_version = validate[version];
		...
		*/
		if (!schemaAdded) {
			...
			schemajson = JSON.parse(schema.toString());
		...
		doValidate(schemajson, json, validated_version, file, str, success, failure);
	} else {
		doValidate(schemajson, json, validated_version, file, str, success, failure);
	}
}
...
export default function validateJSON(files) {
	...
	for (let f in files) {
		try {
			...
			let version = json.X3D["@version"];
			loadSchema(json, file, str, doValidate, function() {
